...
**Explicit Outcome Variables:** Ordinal or Binary Categorical Variable for Coral Health (Good/Bad or a scale if we can find it)
...
We could predict the stay of patient in the hospital using linear regression techniques. This would include analyzing certain predictors looking for signficant factors to a longer or shorter stay. 
...
**About the dataset:**
This main dataset was found on gigasheet.com. It is combined songs from both Youtube and Spotify. It contains 20,594 rows of data.
...
**Limitations:** The dataset is web scraped data, so it is lacking in the reliability area. However, it is supported by its large quantity of data and mixture between different websites. In addition it contains a lot of variables ranging from categorical and continuous. 


**How do we plan to combat this?** We have found multiple datasets that we can use to merge on Song Title. We hope these datasets will add variables we can use like playlist genre, song genre, playlist title, etc. We plan to merge these and clean it as requried. Below is an example of a simple merge from another webscraped dataset found on kaggle.com

#### 1st Idea:

We would like to look into if we can predict genre based off all of these musical measures.In addition to all the possible variables, we would specificaly like to look at factors like loudness, speechiness, danceability, and even year.

**Question:** What types of factors for songs are the most crucial in predicting genre? 

**Explicit Outcome Variable:** Categorical Genre

#### 2nd Idea:

We would like to see if there is any correlation between these variables and length of song. It would be interesting to identify if certain variables can lead to longer or shorter songs. This would help artists and listeners know what they are getting into time wise when they pick certain music to listen to. 

**Question:** Can we predict how long a song will be based on multiple song qualities?

**Explicit Outcome Variable:** Length of song (ms)

##### Other notes:
___
- After coming up with the datasets and ideas we did find out that other groups had similar         datasets. However, we hope our take on merging multiple datasets to get specific variables of interests to us sets us apart from the crowd. 

- In addition, we would be ineterested in doing popularity or likes as a continuous outcome variable. However we wanted to stick out and have a different take on these datasets, rather than what was done online already.
